...
import com.yunwoo.cybershop.annotation.updateIgnore;
import com.yunwoo.cybershop.annotation.table;
import com.yunwoo.cybershop.annotation.updateIgnore;
...
public class ObjectSQLUtil {
	 ...
	 * @return
	 */
	public static String getMax(Class<?> obj) {
		String key = getMaxKey(obj.getName());
		if(objSQLMap.containsKey(key)){
			return objSQLMap.get(key); 
		}
		StringBuffer sql = new StringBuffer();
		...
		sql.append("select max(id) as id from ");
		sql.append(tableName);
		objSQLMap.put(key, sql.toString());
		return sql.toString();
	}
	...
	private static String getMaxKey(String name){
		return name + "_max";
	}
